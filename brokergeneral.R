...
"
  ...
  gtsave(paste0("data/brokerGeneral_", gsub("-", "_", fecha), "_jor_", jornada, ".png"), vwidth = 3000, vheight = 1500, expand = 200)
...
"
  ...
  gtsave(paste0("data/General_", gsub("-", "_", fecha), "_jor_", jornada, ".png"), vwidth = 3000, vheight = 1500, expand = 200)

#######################################################################################################################################################
################################ clasifición jornada ##################################################################################################
#######################################################################################################################################################

#bucle para extraer las paginas se para cuando totalPlayerPoints suma 0 o menos
resultados <- list()
...
for (i in 1:3081) {
  ...
  #pausa de 5 seg cada 500 páginas
  if (i %% 500 == 0) {
    Sys.sleep(5)
  }
}
...
fromJSON(txt = content(
  ...
  select(arrow, combo_img, username, nameTeam,
         totalPlayerPoints, rk1 = position, pointsgeneral,
         rk2 = positiongneral) %>%
  gt() %>%
  cols_align(
    align = "center",
