...
broker_general <- fromJSON(txt = content(
  GET(
    url = Sys.getenv("URL_BROKER"),
...
broker_general %>%
  ...
  gt_merge_stack(
    col1 = nameTeam, col2 = username,
    font_size = c("18px", "14px")
  ) %>%
  cols_label(
    arrow = "",
    combo_img = "",
...
"
  ...
  tab_header(
    ...
    subtitle = md(glue("<span style='font-weight:400;color:#8C8C8C;fon t-size:20px'>Lideres en BrokerBasket TOP20 en el SuperManager 25/26 hasta la jornada 34</span>"))
  ) %>%
  tab_source_note(
  ...
  gtsave(paste0("png/brokerGeneral_jor_", jornada, ".png"), vwidth = 3000, vheight = 1500, expand = 200)
